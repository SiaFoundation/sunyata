...
func merkleProofRoot(leafHash sunyata.Hash256, leafIndex uint64, proof []sunyata.Hash256) sunyata.Hash256 {
	root := leafHash
	for i, h := range proof {
		if leafIndex&(1<<i) == 0 {
...
type stateObject struct {
	objHash   sunyata.Hash256
	leafIndex uint64
	flags     uint64
	proof     []sunyata.Hash256
}

func (so stateObject) leafHash() sunyata.Hash256 {
...
func (so stateObject) proofRoot() sunyata.Hash256 {
	return merkleProofRoot(so.leafHash(), so.leafIndex, so.proof)
}

func (so stateObject) truncatedProofRoot(n int) sunyata.Hash256 {
	return merkleProofRoot(so.leafHash(), so.leafIndex, so.proof[:n])
}

func outputStateObject(o sunyata.Output, flags uint64) stateObject {
...
func objectsByTree(txns []sunyata.Transaction) [64][]stateObject {
...
func updateProof(proof []sunyata.Hash256, leafIndex uint64, updated *[64][]stateObject) {
...
// tree.
func (sa *StateAccumulator) addNewObjects(objects []stateObject) [64][]sunyata.Hash256 {
	...
	for i := range objects {
		...
		objects[i].proof = make([]sunyata.Hash256, 0, trailingOnes(sa.NumLeaves))
...
func (sa *StateAccumulator) updateExistingObjects(objects []stateObject) [64][]stateObject {
	var recompute func(i, j uint64, objects []stateObject) sunyata.Hash256
	recompute = func(i, j uint64, objects []stateObject) sunyata.Hash256 {
...
func ExpandMultiproof(txns []sunyata.Transaction, proof []sunyata.Hash256) {
	var expand func(i, j uint64, objects []stateObject) sunyata.Hash256
	expand = func(i, j uint64, objects []stateObject) sunyata.Hash256 {
		height := bits.TrailingZeros64(j - i)
		if len(objects) == 0 {
