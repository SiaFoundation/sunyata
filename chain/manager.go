...
type ManagerStore interface {
	...
	Checkpoint(index sunyata.ChainIndex) (consensus.Checkpoint, error)
	Header(index sunyata.ChainIndex) (sunyata.BlockHeader, error)

	ExtendBest(index sunyata.ChainIndex) error
	RewindBest() error
	BestIndex(height uint64) (sunyata.ChainIndex, error)
...
func (m *Manager) Tip() sunyata.ChainIndex {
	return m.TipState().Index
}

// Block returns the block at the specified index.
func (m *Manager) Block(index sunyata.ChainIndex) (sunyata.Block, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *Manager) State(index sunyata.ChainIndex) (consensus.State, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	c, err := m.store.Checkpoint(index)
	return c.State, err
...
func (m *Manager) History() ([]sunyata.ChainIndex, error) {
	...
	var history []sunyata.ChainIndex
	for {
...
func (m *Manager) HeadersForHistory(headers []sunyata.BlockHeader, history []sunyata.ChainIndex) ([]sunyata.BlockHeader, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *Manager) AddHeaders(headers []sunyata.BlockHeader) (*consensus.ScratchChain, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	if len(headers) == 0 {
		return nil, nil
...
func (m *Manager) AddTipBlock(b sunyata.Block) error {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *Manager) applyTip(index sunyata.ChainIndex) error {
	c, err := m.store.Checkpoint(index)
	if err != nil {
...
func (m *Manager) reorgPath(a, b sunyata.ChainIndex) (revert, apply []sunyata.ChainIndex, err error) {
	...
	rewind := func(index *sunyata.ChainIndex) bool {
...
// updated accordingly.
func (m *Manager) AddSubscriber(s Subscriber, tip sunyata.ChainIndex) error {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *Manager) UpdateElementProof(e *sunyata.StateElement, a, b sunyata.ChainIndex) error {
	m.mu.Lock()
	defer m.mu.Unlock()
